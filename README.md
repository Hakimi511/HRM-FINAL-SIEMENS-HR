...
- 成绩按**学员身份**（姓名/区域/工号）归档，可**一键导出**交给 HR 汇总
- 数据保存在浏览器本地（localStorage），无需后端、保护隐私

### 🛡️ HR 管理后台（管理员）
> 口令进入（演示口令：`siemens2025`），面向西门子 HR / 培训管理员

- **总览 KPI**：参训学员数、考核次数、平均正确率、整体合格率
- **达标环形图** + **各销售区域对比**：识别区域间能力差异
- **知识薄弱点分析**：按能力模块/知识分类计算正确率，自动给出**培训建议**
- **难题排行 Top 8**：全员错得最多的题目，反推内容难点
- **学员明细表 + 排行榜**：姓名/区域/考核次数/均分/状态，一目了然
- **数据管理**：导入学员成绩、导出 CSV 报表 / JSON 备份、一键载入演示数据

## 👤 双模式

| 模式 | 进入方式 | 能力 |
|---|---|---|
| **学员模式**（默认） | 首次访问登记姓名+销售区域 | 答题考核、学习资料、个人成绩与错题本 |
| **HR 管理后台** | 顶部「🛡 HR 管理后台」+ 口令 `siemens2025` | 全员学习考核数据分析驾驶舱 |

> 纯前端无后端，跨设备数据通过「学员导出 → HR 导入」汇总；正式部署可对接企业 LMS / 数据库实现集中统计。
...
├── js/
│   ├── app.js              # 哈希路由 + 身份登记 + HR后台门控 + PDF弹窗
│   ├── home.js             # 首页/仪表盘
│   ├── exam.js             # 考核引擎（组卷/答题/判分，记录身份与逐题明细）
│   ├── library.js          # 学习资料库
│   ├── records.js          # 我的成绩（含导出）
│   ├── admin.js            # HR 管理后台（数据分析驾驶舱）
